    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for index in range(sentence_length):
            ...
            if index < window_size:
                for i in range(window_size-index):
                    concatenated_vector.extend(start)
                for i in range(index):
                    concatenated_vector.extend(sentence[i])
            else:
                for i in range(index-window_size, index):
                    concatenated_vector.extend(sentence[i])

            concatenated_vector.extend(sentence[index])

            if (index+window_size) >= sentence_length:
                for i in range(index+1, sentence_length):
                    concatenated_vector.extend(sentence[i])
                for i in range(window_size - (sentence_length - 1 - index)):
                    concatenated_vector.extend(end)
            else:
                for i in range(index+1, index+1+window_size):
                    concatenated_vector.extend(sentence[i])
            windowed_data.append((concatenated_vector, labels[index]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        """
        #print '\n feed_dict input shape: ', inputs_batch.shape
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_matrix = tf.get_variable('embedding_matrix', dtype=tf.float32, initializer=tf.constant(self.pretrained_embeddings))
        input_embeddings = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder) #shape: (None, n_window_features, embedding_size)
        embeddings = tf.reshape(input_embeddings,
                                shape=[-1, self.config.n_window_features * self.config.embed_size],
                                name='embedding_reshape')
        ### END YOUR CODE
        tf.summary.histogram('{}_histogram'.format('embedding_matrix'), embedding_matrix)
    ...
    def add_prediction_op(self):
        ...
        embedded_x = self.add_embedding()
        dropout_rate = self.dropout_placeholder
        ### YOUR CODE HERE (~10-20 lines)
        W_matrix = tf.get_variable('W_matrix',
        ...
        U_matrix = tf.get_variable('U_matrix',
        ...
        hidden_layer = tf.nn.relu(tf.add(tf.matmul(embedded_x, W_matrix), b1), name='hidden_layer')
        dropped_hidden_layer = tf.nn.dropout(hidden_layer, keep_prob=dropout_rate, name='dropped_hidden_layer')
        pred = tf.reshape(tf.add(tf.matmul(dropped_hidden_layer, U_matrix),  b2),
                          shape=[-1, self.config.n_classes],
                          name='pred_reshape')

        tf.summary.histogram('{}_histogram'.format('W_matrix'), W_matrix)
        tf.summary.histogram('{}_histogram'.format('U_matrix'), U_matrix)
        #tf.summary.histogram('{}_histogram'.format('pred'), pred)
    ...
    def train_on_batch(self, sess, inputs_batch, labels_batch):
        ...
        #_, loss, summary = sess.run([self.train_op, self.loss, summary_op], feed_dict=feed)
        #self.global_step += 1
    ...
    def __init__(self, helper, config, pretrained_embeddings, report=None):
        ...
        self.build()
        # create a summary writer for model
        self.writer = tf.summary.FileWriter('model_log', graph=tf.get_default_graph())
        self.global_step = 0
...
def test_make_windowed_data():
    ...
    sentences = [[[1,1], [2,2], [3,3], [4, 4]]]
    sentence_labels = [[1, 2, 3, 4]]
    data = zip(sentences, sentence_labels)
    w_data = make_windowed_data(data, start=[0,0], end=[9,9], window_size=2)

    assert len(w_data) == sum(len(sentence) for sentence in sentences)
    print 'the raw w_data: ', w_data

    assert w_data == [
        ([0,0] + [0,0] + [1,1] + [2,2] + [3,3], 1,),
        ([0,0] + [1,1] + [2,2] + [3,3] + [4,4], 2,),
        ([1,1] + [2,2] + [3,3] + [4,4] + [9,9], 3,),
        ([2,2] + [3,3] + [4,4] + [9,9] + [9,9], 4,),
        ]
