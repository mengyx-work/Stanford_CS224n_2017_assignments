...
class NERModel(Model):
    ...
    def evaluate(self, sess, examples, examples_raw):
        ...
        for _, labels, labels_  in self.output(sess, examples_raw, examples):
            #print 'the prediction results: ', labels, labels_
    ...
    def output(self, sess, inputs_raw, inputs=None):
        ...
        prog = Progbar(target=1 + int(len(inputs) / self.config.pred_batch_size))

        #for i, batch in enumerate(minibatches(inputs, self.config.batch_size, shuffle=False)):
        for i, batch in enumerate(minibatches(inputs, self.config.pred_batch_size, shuffle=False)):
            # Ignore predict
