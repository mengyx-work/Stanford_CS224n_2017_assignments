...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            h = tf.convert_to_tensor(state, name='h')
            #print '\n the x shape from cell: ', x.shape
            new_state = tf.sigmoid(tf.matmul(x, W_x, name='input_matmul') + tf.matmul(h, W_h, name='hidden_matmul') + b)
            ### END YOUR CODE ###
