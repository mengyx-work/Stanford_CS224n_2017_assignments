...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padded_sentence, padded_labels, mask, sentence_length = [], [], [], len(sentence)
        for index in range(max_length):
            if index < sentence_length:
                padded_sentence.append(sentence[index])
                padded_labels.append(labels[index])
                mask.append(True)
            else:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedded_x = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
        embeddings = tf.reshape(embedded_x, shape=[-1, self.max_length, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable('U',
                            shape=[self.config.n_features * self.config.embed_size, self.config.n_classes],
                            dtype=tf.float32,
        ...
        init_h = tf.reshape(tf.zeros(shape=[self.config.hidden_size,], dtype=tf.float32), [-1, self.config.hidden_size])
        x_array = tf.unstack(x, axis=1)
        scope_name_, output_array = 'rnn_unroll', []
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)
                if time_step == 0:
                    raw_output, new_state = cell(x_array[time_step], init_h, scope=scope_name_)
                else:
                    tf.get_variable_scope().reuse_variables()
                    raw_output, new_state = cell(x_array[time_step], new_state, scope=scope_name_)
                output = tf.add(tf.matmul(raw_output, U), b2)
                output_array.append(output)
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
                preds = tf.stack(output_array, axis=1)
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        raw_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds, name='raw_loss')
        loss = tf.reduce_mean(tf.boolean_mask(tensor=raw_loss, mask=self.mask_placeholder, name='mask_loss'))
        ### END YOUR CODE
        return loss
